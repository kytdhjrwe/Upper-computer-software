...
class physiological_serial(threading.Thread):
    def __init__(self):
        ...
        self.leg_point1_data = []
        self.leg_point2_data = []
        self.leg_point3_data = []
        self.leg_point4_data = []
        self.leg_point5_data = []
        self.leg_point6_data = []
        self.leg_point7_data = []
        self.leg_point8_data = []

        self.leg_point9_data = []
        self.leg_point10_data = []
        self.leg_point11_data = []
        self.leg_point12_data = []
        self.leg_point13_data = []
        self.leg_point14_data = []
        self.leg_point15_data = []
        self.leg_point16_data = []

        self.serLeft = serial.Serial()  # initialize the serial port
        self.serRight = serial.Serial()  # initialize the serial port
        self.serLeft3 = serial.Serial()  # initialize the serial port
        self.serRight4 = serial.Serial()  # initialize the serial port
        ...
        self.checkHead3 = deque(maxlen=2)  # 只保留四个元素的列表
        self.checkHead3.append('0')
        self.checkHead3.append('0')

        self.checkHead4 = deque(maxlen=2)  # 只保留四个元素的列表
        self.checkHead4.append('0')
        self.checkHead4.append('0')
            ...
            if self.checkHead[0]==205 and self.checkHead[1]==171:
                ...
                if end_array[0]==222 and end_array[1]==188:
                    ...
                    for i in range(8):
                        ...
                        # pointlistLeft.append(int(binascii.b2a_hex(onepoint).decode('utf-8').upper(), 16) / 4096.0 * 3.3 )
                        p = p + 2
    ...
    def receive_data2(self):  # Overwrite run() method, put what you want the thread do here
        print("The receive_data threading is start")
            ...
            if self.checkHead2[0]==205 and self.checkHead2[1]==171:
                ...
                if end_array[0]==222 and end_array[1]==188:
                    ...
                    for i in range(8):
                        ...
                        # print('onepoint1',onepoint1)
                        # print('onepoint',onepoint)
                        pointlistRight.append(int(binascii.b2a_hex(onepoint).decode('utf-8').upper(), 16) / 4096.0 * 3.3 )
                        p = p + 2
    ...
    def receive_data3(self):  # Overwrite run() method, put what you want the thread do here
        print("The receive_data threading is start")
        ...
        while not self.thread_stop:
            ...
            # int(x, base=10): x为字符串或数字，base为进制数，默认十进制；若使用base，x需使用字符串形式；返回十进制整形数据
            # Hex_head_frame = int(binascii.b2a_hex(Head_frame).decode('utf-8'), 16)
            Head_frame3 = self.serLeft3.read(1)  # 读取头帧,从串行端口读取1个字节
            self.checkHead3.append(Head_frame3[0])
            # print('self.checkHead', self.checkHead)
            if self.checkHead3[0]==205 and self.checkHead3[1]==171:
                fenjio = self.serLeft3.read(2)
                timeStamp = self.serLeft3.read(4)
                Value_array = self.serLeft3.read(16)
                end_array = self.serLeft3.read(2)
                ...
                if end_array[0]==222 and end_array[1]==188:
                    ...
                    self.timeLeft3.append(thetime)
                    self.leg_point1_data.append(pointlistLeft[0])
                    self.leg_point2_data.append(pointlistLeft[1])
                    self.leg_point3_data.append(pointlistLeft[2])
                    self.leg_point4_data.append(pointlistLeft[3])
                    self.leg_point5_data.append(pointlistLeft[4])
                    self.leg_point6_data.append(pointlistLeft[5])
                    self.leg_point7_data.append(pointlistLeft[6])
                    self.leg_point8_data.append(pointlistLeft[7])


    def receive_data4(self):  # Overwrite run() method, put what you want the thread do here
        print("The receive_data threading is start")

        while not self.thread_stop:

            Head_frame4 = self.serRight4.read(1)  # 读取头帧,从串行端口读取1个字节
            self.checkHead4.append(Head_frame4[0])
            if self.checkHead4[0]==205 and self.checkHead4[1]==171:

                fenjio = self.serRight4.read(2)
                timeStamp = self.serRight4.read(4)
                Value_array = self.serRight4.read(16)
                end_array = self.serRight4.read(2)
                ...
                if end_array[0]==222 and end_array[1]==188:
                    ...
                    self.timeRight4.append(thetime)
                    self.leg_point9_data.append(pointlistRight[0])
                    self.leg_point10_data.append(pointlistRight[1])
                    self.leg_point11_data.append(pointlistRight[2])
                    self.leg_point12_data.append(pointlistRight[3])
                    self.leg_point13_data.append(pointlistRight[4])
                    self.leg_point14_data.append(pointlistRight[5])
                    self.leg_point15_data.append(pointlistRight[6])
                    self.leg_point16_data.append(pointlistRight[7])
    ...
    def turnOn(self):
        ...
        if self.serLeft.port != None:
            self.serLeft.open()
            self.t1 = threading.Thread(target=self.receive_data)
            self.t1.setDaemon(True)
            self.t1.start()

        if self.serRight.port != None:
            self.serRight.open()
            self.t2 = threading.Thread(target=self.receive_data2)
            self.t2.setDaemon(True)
            self.t2.start()

        if self.serLeft3.port != None:
            self.serLeft3.open()
            self.t3 = threading.Thread(target=self.receive_data3)
            self.t3.setDaemon(True)
            self.t3.start()

        if self.serRight4.port != None:
            self.serRight4.open()
            self.t4 = threading.Thread(target=self.receive_data4)
    ...
    def getData(self):
        return self.timeLeft, self.point1_data, self.point2_data,self.point3_data,\
               ...
               self.timeLeft3, self.leg_point1_data, self.leg_point2_data, self.leg_point3_data, \
               self.leg_point4_data, self.leg_point5_data, self.leg_point6_data, self.leg_point7_data, self.leg_point8_data, \
               self.timeRight4, self.leg_point9_data, self.leg_point10_data, self.leg_point11_data, \
               self.leg_point12_data, self.leg_point13_data, self.leg_point14_data, self.leg_point15_data, self.leg_point16_data
    ...
    def setLeftPort3(self, str):
        print('左腿port修改为 ',str)
        self.serLeft3.port = str

    def setRightPort4(self, str):
        print('右腿port修改为 ', str)
        self.serRight4.port = str

    def setBaud(self, str):
        print('baud修改为 ',str)
        self.serLeft.baudrate = str
        self.serRight.baudrate = str
        self.serLeft3.baudrate = str
        self.serRight4.baudrate = str
